...
function fixStm(n, parentScript) {
  ...
  case TRY: // turn the varName of each catch-clause to a node called exvar
    ...
    n.catchClauses.forEach(function(clause) {
      ...
      catchClauses.push(clause); // record for scoped tags file generation
      ans1 = ans1 || ans2;
    });
...
var catchClauses; // keep record for scoped tags
var commonJSmode;
...
function getTags(ast, pathtofile, lines, options) {
  ...
  options.localVars && catchClauses.forEach(function(clause) {
    tags.push({ name : clause.varName,
                tagfile : pathtofile,
                addr : regexify(lines[clause.lineno - 1]),
                kind : "ve",
                type : "(exception)",
                lineno : clause.lineno.toString(),
                sortno : clause.lineno.toString(),
                scope  : clause.startPos+'-'+clause.endPos
              });
  });
  return tags;
}
